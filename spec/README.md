...
#### 設定の永続化方針(opt-in サイドカー方式、2026-07-18 決定・実装済み (20))
...
- 永続化対象は `showHidden` のみ。スキーマの知識は `src/lib/settingsFile.ts`
  の 1 箇所に集約(Rust はファイル内容を不透明な文字列として read/write する
  だけ)。パースは未知キーを無視し、既知キーの欠落・型違いはデフォルトに
  フォールバックするため、将来のキー追加に耐える。
- UI はツールバーの歯車ドロップダウンメニュー(@kobalte/core、コンテキスト
  メニューとメニュースタイルを共有)。「Show hidden files」と
  「Remember settings」の 2 つのチェック項目(各項目にアイコン付き)を持ち、
  隠しファイル表示の切替もここに統合した(単独の目アイコンボタンは廃止)。
- 永続化は「Remember settings」のチェックで ON/OFF する。ON で
  `voyager.json` を作成し、以後の設定変更を自動保存。OFF でファイルを削除
  してセッションモードに戻る。`Mod+S` は ON にするショートカット(フィルタ
  入力中も有効)。ON/OFF 操作の失敗はエラーバナーに表示し、自動保存の失敗は
  無視する。経緯: 当初はフロッピーアイコンの即時保存ボタンだったが
  「選択中のファイルを保存」と誤読されるため、状態が見えるチェック式に
  変更した。
- 壊れた・空のファイルは無視してセッションモードで起動し、自動保存も有効化
  しない(ユーザーが調べたいかもしれないファイルを勝手に上書きしないため)。
  「Remember settings」を ON にし直せば正しい内容で修復される。
- macOS バンドル判定は `#[cfg(target_os)]` でなくパス形状
  (`<Name>.app/Contents/MacOS/`)の検出で行う。全 OS でユニットテスト可能で、
  dev モード(`target/debug/voyager`)は自然に exe と同じディレクトリに
  フォールバックする。
...
| 20 | Settings persistence | opt-in サイドカー `voyager.json`(showHidden のみ、歯車メニューの Remember settings チェックで ON/OFF、ON 中は自動保存) |
