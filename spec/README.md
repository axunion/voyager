...
| 05 | [File operations](05-file-operations.md) — Rename / New Folder / New File | 中大 | 完了 | |
| 06 | [Path bar](06-path-bar.md) — パンくず + パス入力 | 小 | 完了 | 02 以降なら入替自由 |
| 07 | [Filter](07-filter.md) — 名前フィルタ | 小 | 完了 | 02 以降なら入替自由 |
| 08 | [In-app DnD 拡張](08-dnd-in-app.md) — ツリー・タブへのドロップ | 中 | 完了 | |
...
| 11 | [Tab hover switch](11-tab-hover-switch.md) — ドラッグホバーでのタブ自動切替 | 中 | 完了 | ドラッグ元タブを裏で保持するアーキテクチャに変更(macOS で手動検証済み) |
...
08 ─→ 09 dnd-os-drop      (dragDropEnabled 反転の検証を全 D&D 面が揃った後に 1 回で行う。回帰により保留)
09 ─→ 10 dnd-drag-out     (保留)
08 ─→ 11 tab-hover-switch (08 の Future work。09/10 とは独立)
```
