---
name: code-reviewer
description: Read-only reviewer for the current working diff. Checks convention adherence (the conventions section of spec/README.md) and stack-specific pitfalls (SolidJS reactivity, Rust/Tauri, IPC mirroring).
...
1. Read the 実装規約 (conventions) section of `spec/README.md` in full.
2. Run `git diff --stat` and `git diff` to see the working changes (include
   staged changes: `git diff HEAD`).
3. Perform the checks below in priority order.

## Checks

### 1. Convention adherence

Compare the diff against every conventions subsection of `spec/README.md`:
error message format, IPC rules, Rust backend rules, store rules, component
rules, CSS rules, UI library rules, and test policy. Also check the product
invariants in `CLAUDE.md` (zero persistence, no OS clipboard, no content
display, no new runtime dependencies).

### 2. Stack-specific pitfalls
...
(invariant violations first, then correctness, then style).
